...
exports.uploadImage = async (req, res) => {
    ...
    const userId = req?.technician._userId;
    ...
    try {
        ...
        const uploadImages = await new Promise((resolve, reject) => {
            images.forEach((image, index, array) => {
                let ref = `technicians/${userId}/images/${image.originalname}`;
    ...
    try {
        await new Promise((resolve, reject) => {
            fileUrls.forEach( async (url, index, array) => {
                ...
                const rootPath = `technicians/`;
                const userId = getUserIdFromFilePath(filePath, rootPath);
        
                if (userId !== req.technician._userId.toString())
                    return res.status(400).json({
                        status: 400,
    ...
    try {
        ...
        const uploadDocs = await new Promise((resolve, reject) => {
            documents.forEach((document, index, array) => {
                let ref = `technicians/${userId}/documents/${document.originalname}`;
